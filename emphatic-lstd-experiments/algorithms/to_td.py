"""
True Online Temporal Difference Learning -- TD(λ) 
"""
import numpy as np
...
class TrueOnlineTD:
    ...
    def update(self, fvec, R, fvec_p, alpha, gamma, lmbda, rho=1):
        ...
        new_theta = self.theta + alpha*delta*self.z + (self.z - alpha*rho*fvec)*np.dot(self.theta - self.old_theta, fvec)
        self.old_theta, self.theta = self.theta, new_theta
        return delta
